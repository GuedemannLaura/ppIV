...
scenario               <- "1a"                                                  # please change name of scenario here (this will determine where results are saved)
switch_probabilities   <- c(0.7, 0.4)

# chose parameters for scenarios
n_i_per_provider       <- c(24, 108, 408)[1]
beta_PP_same           <- c(TRUE, FALSE)[1]
change_in_PP           <- c("none", "some", "all")[2]
PP_change_type         <- c("abrupt", "smooth")[1] 
missingness_indata     <- c("noNAs","MCAR","MNAR")[1]
...
#s <- 5
  ...
  var_Y_nst[s]     <- var(Y)                                                    # variance of the outcome variable
  ...
  IV_PP_cc_results     <- TSLS_continuousY_fun(data = data_ccW1, Y = "Y", W = c("W1", "W2"), X = "X", Z = "PP")
  ...
  IV_PP_results     <- TSLS_continuousY_fun(data = data, Y = "Y", W = c("W1", "W2"), X = "X", Z = "PP")
