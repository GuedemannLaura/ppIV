# ------------------------------------------------------------------------------
# Extension to Ertefaie et al. 2017  (IV_ePP_rirs) -----------------------------
# ------------------------------------------------------------------------------


# Step 1: 

IVePP_rirs_fit1 <- glmer(X ~ W1_wNA + W2 + tx_time + (tx_time|prov_ID), family = binomial("logit"), data = data_ccW1)            

b_intercept_rirs    <- summary(IVePP_rirs_fit1)$coef["(Intercept)", "Estimate"]
b_T_rirs            <- summary(IVePP_rirs_fit1)$coef["tx_time", "Estimate"]
b0_j_hat_rirs       <- ranef(IVePP_rirs_fit1)$prov_ID$"(Intercept)"
b1_j_hat_rirs       <- ranef(IVePP_rirs_fit1)$prov_ID$"tx_time"


# Data cleaning step:

# Step 2 is calculated on the full dataset but we need to exclude physicians which are excluded 
# because all of their patient data on W1 is NA
# for these patients no Z_j_ePP_rirs_all_same was estimated
...
data_ePP2_rirs$fitted         <- data_ePP2_rirs$b_int + data_ePP2_rirs$b0_hat + (data_ePP2_rirs$b_T + data_ePP2_rirs$b1_hat)*data_ePP2_rirs$tx_time 
